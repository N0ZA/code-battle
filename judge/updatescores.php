...
if ($_SESSION['is_team']==1){
    $query3 = "SELECT td.T_id, td.TName, SUM(s.Score) AS score
    FROM team_data td
    LEFT JOIN team_scores s ON td.T_id = s.T_id AND s.J_id = :J_id
    WHERE td.H_id = :H_id GROUP BY td.T_id, td.TName HAVING score!=0;";

}else{
    $query3 = "SELECT sd.P_id, sd.PName, SUM(s.Score) AS score 
    FROM solo_data sd
    LEFT JOIN solo_scores s ON sd.P_id = s.P_id AND s.J_id = :J_id
    WHERE sd.H_id = :H_id GROUP BY sd.P_id, sd.PName HAVING score != 0;";
}
...
<html>
    ...
    <script>
        function CardClick(Tcard) {
            var id=Tcard.getAttribute('id');
...
<div class="button-container">
 ...
 if(!empty($result3)) { ?> 
    <h1 style="text-align:center;margin-top: 90px;">Select participant to change scores. </h1>  
        <div class="card-container">
            <?php foreach($result3 as $row) { 
                // echo $row['T_id'];?>
                <!-- setting T_id as the id of each card -->
    ...
    <?php } else { ?>
        <h2 style="text-align: center;">No participants graded by you yet</h2>
    <?php }}?>
    <footer>
